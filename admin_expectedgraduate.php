<?php
 ...
 $connect2 = mysqli_connect('localhost','root','applemac','macDB');
 if (!$connect2) {
   die(' Mac 데이터베이스 접근에 문제가 있습니다.' . mysqli_error($connect2));
 }

mysqli_select_db($connect, "macDB");
mysqli_select_db($connect2, "macDB");

$query_mysql3="SELECT *  FROM member, bookAuth, englishAuth
WHERE member.studentId = bookAuth.studentId and member.studentId = englishAuth.studentId and bookAuth.studentId = englishAuth.studentId and member.year >3";
       ...
       <div class="mdl-grid">
           ...
           <div class="mdl-list mdl-shadow--2dp mdl-color--white">
             <div class="mdl-list__item">
               <span class="mdl-list__item-primary-content">
                 ...
                 <span><strong>졸업 예정 학생 조회</strong><!--?php echo $member[3] . " (" . $member[2] . ")"; ?--></span>
               </span>
         ...
         <table class="mdl-data-table mdl-js-data-table mdl-data-table--unselectable mdl-shadow--2dp mdl-cell mdl-cell--10-col-desktop mdl-cell--8-col-tablet mdl-cell--4-col-phone mdl-cell--1-offset-desktop" style="text-align: center;">
                 ...
                 <tr>
                     ...
                     <th style="text-align: center;">독서<br>인증</th>
                 ...
                 <tbody>
                     ...
                     $result4 = mysqli_query($connect, $query_mysql3);

                     for($i=0; $i < mysqli_num_rows($result4); $i++ ){
                       $alertFlag = 0;
                       $result_each3 = mysqli_fetch_assoc($result4);
                       if ($result_each3["gradFlag"]==0) {
                         $userMajor = $result_each3["major"];
                         $userId = $result_each3["studentId"];
                         $userAdYear = $result_each3["adYear"];

                         $query_mysql11="SELECT * FROM graduation WHERE major = '$userMajor' and adYear = '$userAdYear'";
                         $totalCredit = mysqli_query($connect, $query_mysql11);
                         $result_credit = mysqli_fetch_assoc($totalCredit);

                         $query_mysql12="SELECT *  FROM subject JOIN complete ON subject.subKey = complete.subjectKey  WHERE complete.studentId = '$userId' and subject.adYear = '$userAdYear' ";
                         $result10 = mysqli_query($connect, $query_mysql12);
                         $mySubjectTotal = mysqli_num_rows($result10);
                         //중핵 필수
                         ...
                         for($j = 0; $j < $mySubjectTotal ; $j++){
                             $result_each8 = mysqli_fetch_assoc($result10);
                             if($result_each8["field"] == 1 ) { //중핵 필수
                                 $coreEs += $result_each8["credit"];
                                 $gradTotal += $result_each8["credit"];
                             }else if($result_each8["field"] == 2){ //중핵 선택
                                 $coreSe += $result_each8["credit"];
                                 $gradTotal += $result_each8["credit"];
                             }else if($result_each8["field"] == 3){ //기초 교양
                                   $majorBasic += $result_each8["credit"];
                                   $gradTotal += $result_each8["credit"];
                             }else if($result_each8["field"] == 4){ //전공 필수
                                     $majorEs += $result_each8["credit"];
                                     $gradTotal += $result_each8["credit"];
                             }else if($result_each8["field"] == 5){ //전공 선택
                                   $majorSe += $result_each8["credit"];
                                   $gradTotal += $result_each8["credit"];
                             }
                         }

                         $majorEs2 = $majorEs;
                         if($majorEs >= $result_credit["majorEs"]){
                           $tmp = $majorEs - $result_credit["majorEs"];
                         ...
                         $leftOver = $result_credit["gradTotal"]-$gradTotal;
                         if ($leftOver<=0)
                          $leftOver = 'O';
                         $sem = 9-$result_each3["semester"];
                         $semLeft = $sem*15;
                         if ($leftOver>$semLeft)
                          $alertFlag++;
                       }
                       ...
                       if ($result_each3["score"]<800)
                          $alertFlag++;
                        if ($alertFlag>1)
                          echo "<tr bgcolor=\"red\" onclick=\"SearchStudent(".$i.");return false;\">";
                          else
                            echo "<tr onclick=\"SearchStudent(".$i.");return false;\">";

                      //  echo "<td>".$result_each3["studentId"]."</td>";
                        echo "<td style=\"text-align: center;\" id=sample".$i.">".$result_each3["studentId"]."</td>";
                        echo "<td style=\"text-align: center;\">".$result_each3["name"]."</td>";
                        echo "<td style=\"text-align: center;\">".$result_each3["year"]."</td>";
                        if ($result_each3["semester"]%2 == 0)
                        ...
                        if ($result_each3["gradFlag"]) {
                          ...
                          echo "<td style=\"text-align: center;\">".$leftOver."</td>";
                        }
         ...
         for(; $i<= $total; $i++){
           echo "<a href=\"admin_expectedgraduate.php?page=".$i."\">".$i." </a>";
         }
         echo "</div>";
          ?>
          </td>

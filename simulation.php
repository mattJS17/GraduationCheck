<?php
  ...
  $query_mysql="SELECT * FROM graduation WHERE major = '$member[3]' and adYear = '$member[8]' ";
...
<script>
  ...
  function calcExpected(div, credit, key) {
    ...
    if (div == 1) {
      ...
      c = document.getElementById("expectedMajorEs").innerHTML;
      d = document.getElementById("expectedGradTotal").innerHTML;
      e = document.getElementById("expectedMajorSe").innerHTML;
      if (document.getElementById(key).bgColor == "grey") {
        bool = -1;
        document.getElementById(key).bgColor = "white";
      } else {
      ...
      if (Number(c) + bool*Number(credit) > Number(a)) {
        ...
        if(Number(c) >= Number(a)) {
          tmp = bool*Number(credit);
        } else {
          tmp = Math.abs(Number(a) - (Number(c) + bool*Number(credit)));
        }

        var innerCheck = "<i class=\"material-icons style=\"font-size: 10px\">done</i>";
        document.getElementById("neededMajorEs").innerHTML = innerCheck;

        if(isNaN(  document.getElementById("neededMajorSe").innerHTML )){

          var needNan = Number(a)+Number(b)-(Number(c)+bool*Number(credit))-Number(e);

           if(Number(needNan)>0){
           document.getElementById("neededMajorSe").innerHTML = Number(needNan);
         }

        }else{
        document.getElementById("neededMajorSe").innerHTML = Number(document.getElementById("neededMajorSe").innerHTML) - tmp;
        if(document.getElementById("neededMajorSe").innerHTML <= 0)
           document.getElementById("neededMajorSe").innerHTML = innerCheck;
      ...
      var totalG = document.getElementById("neededGradTotal").innerHTML;
      if(totalG <= 0){
        document.getElementById("neededGradTotal").innerHTML = "<i class=\"material-icons style=\"font-size: 10px\">done</i>";
      }
